...
import com.matez.wildnature.util.handlers.RegistryHandler;
...
public class LeavesAppleBase extends WNBlockLeaves implements IHasModel {
    ...
    NBTTagCompound tag;
    public LeavesAppleBase(String name, Material material){
        super();
            ...
            if (Main.generateRandomInt(0, 35) == 0) {
                ...
                if (!(Main.isTheSameBlockNear(worldIn, pos, false))) {
                    ...
                    if(Main.getStateForSameNearBlock(worldIn,pos).toString().contains("variant=0")){
                        ...
                        if(gen==0){
                            worldIn.setBlockState(pos, ModBlocks.APPLE_LEAVES.getDefaultState().withProperty(VARIANT, 1));
                        }else{
                            worldIn.setBlockState(pos, ModBlocks.APPLE_LEAVES.getDefaultState().withProperty(VARIANT, 3));
                        }
                    }else{
                        worldIn.setBlockState(pos, ModBlocks.APPLE_LEAVES.getDefaultState().withProperty(VARIANT, 2));
                    }
                }
            }
        }
    ...
    public void normalFruitGen(World worldIn,BlockPos pos){
        ...
        if (gen == 0) {
            ...
            if (worldIn.getBlockState(pos).toString().contains("variant=0")) {
                ...
                updateVariant(worldIn, pos);
                if (BlockVariant != 0) {
                    worldIn.setBlockState(pos, ModBlocks.APPLE_LEAVES.getDefaultState().withProperty(VARIANT, 0));

                    if (BlockVariant == 2) {
                        spawnAsEntity(worldIn, pos, new ItemStack(ModItems.ZIELONE_JABLKO));
                    } else {
                        spawnAsEntity(worldIn, pos, new ItemStack(Items.APPLE));
                    }
                }
