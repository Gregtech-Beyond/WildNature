...
public class LeavesRowanBase extends WNBlockLeaves implements IHasModel {
    ...
    public LeavesRowanBase(String name, Material material){
        super();
    ...
    public void updateTick(World worldIn, BlockPos pos, IBlockState state, Random rand) {
        ...
        if(!placedByPlayer) {
            ...
            if (Main.generateRandomInt(0, 45) == 0) {
                ...
                if (!(Main.isTheSameBlockNear(worldIn, pos, false))) {
                    normalFruitGen(worldIn,pos);

                }else{
                    if(Main.getStateForSameNearBlock(worldIn,pos).toString().contains("variant=0")){
                        normalFruitGen(worldIn,pos);
                    }else if(Main.getStateForSameNearBlock(worldIn,pos).toString().contains("variant=1")||Main.getStateForSameNearBlock(worldIn,pos).toString().contains("variant=3")){
                        int gen = 0;
                        gen = Main.generateRandomInt(0, 1);
                        if(gen==0){
                            worldIn.setBlockState(pos, ModBlocks.ROWAN_LEAVES.getDefaultState().withProperty(VARIANT, 1));
                        }else{
                            worldIn.setBlockState(pos, ModBlocks.ROWAN_LEAVES.getDefaultState().withProperty(VARIANT, 3));
                        }
                    }else{
                        worldIn.setBlockState(pos, ModBlocks.ROWAN_LEAVES.getDefaultState().withProperty(VARIANT, 2));
                    }
                }
            }
        }
    ...
    public void normalFruitGen(World worldIn,BlockPos pos){
        ...
        if (gen == 0) {
            ...
            if (worldIn.getBlockState(pos).toString().contains("variant=0")) {
                ...
                updateVariant(worldIn, pos);
                if (BlockVariant != 0) {
                    worldIn.setBlockState(pos, ModBlocks.ROWAN_LEAVES.getDefaultState().withProperty(VARIANT, 0));

                    if (BlockVariant == 2) {
                        spawnAsEntity(worldIn, pos, new ItemStack(ModItems.ROWAN_CZERWONA));
                    } else {
                        spawnAsEntity(worldIn, pos, new ItemStack(ModItems.ROWAN_POMARANCZOWA));
                    }
                }
