...
public class LeavesCherryWhiteBase extends WNBlockLeaves implements IHasModel {
    ...
    NBTTagCompound tag;
    public LeavesCherryWhiteBase(String name, Material material){
        super();
        ...
        if(continueFruit)
            if(!placedByPlayer) {
                ...
                if (Main.generateRandomInt(0, 35) == 0) {
                    normalFruitGen(worldIn,pos);
                }
            }
    }

    public void normalFruitGen(World worldIn,BlockPos pos){
        if(!worldIn.isAirBlock(pos)) {
            int gen = 0;
            gen = Main.generateRandomInt(0, 4);
            if (gen == 0) {
                worldIn.setBlockState(pos, ModBlocks.CHERRY_WHITE_LEAVES.getDefaultState().withProperty(VARIANT, 1));
            } else if (gen == 1) {
                if (worldIn.getBlockState(pos) == worldIn.getBlockState(pos).withProperty(VARIANT, 0)) {
                    worldIn.setBlockState(pos, ModBlocks.CHERRY_WHITE_LEAVES.getDefaultState().withProperty(VARIANT, 1));
                } else {
                    worldIn.setBlockState(pos, ModBlocks.CHERRY_WHITE_LEAVES.getDefaultState().withProperty(VARIANT, 2));
                }
            } else if (gen == 2) {
                if (worldIn.getBlockState(pos) == worldIn.getBlockState(pos).withProperty(VARIANT, 0)) {

                } else {
                    worldIn.setBlockState(pos, ModBlocks.CHERRY_WHITE_LEAVES.getDefaultState().withProperty(VARIANT, 3));
                }
            } else if (gen == 3) {
                if (worldIn.getBlockState(pos) == worldIn.getBlockState(pos).withProperty(VARIANT, 0)) {

                } else {
                    worldIn.setBlockState(pos, ModBlocks.CHERRY_WHITE_LEAVES.getDefaultState().withProperty(VARIANT, 4));
                }
            } else {
                gen = Main.generateRandomInt(0, 5);
                if (gen == 0) {
                    if (BlockVariant != 0) {
                        if (BlockVariant == 1) {
                            worldIn.setBlockState(pos, ModBlocks.CHERRY_WHITE_LEAVES.getDefaultState().withProperty(VARIANT, 0));
                            spawnAsEntity(worldIn, pos, new ItemStack(ModBlocks.CHERRY_KWIAT));
                        } else {
                            worldIn.setBlockState(pos, ModBlocks.CHERRY_WHITE_LEAVES.getDefaultState().withProperty(VARIANT, 0));
                            spawnAsEntity(worldIn, pos, new ItemStack(ModItems.CHERRY));
                        }
                    }
                }
            }
    ...
    public void updateVariant(World worldIn,BlockPos pos){
        if(worldIn.getBlockState(pos).toString().contains("variant=0")){
            ...
            BlockVariant = 3;
        }else if(worldIn.getBlockState(pos).toString().contains("variant=4")){
            BlockVariant = 4;
        }
    }
