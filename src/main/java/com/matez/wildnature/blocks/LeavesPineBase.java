...
public class LeavesPineBase extends WNBlockLeaves implements IHasModel {
    ...
    NBTTagCompound tag;
    public LeavesPineBase(String name, Material material){
        super();
    ...
    public void updateTick(World worldIn, BlockPos pos, IBlockState state, Random rand) {
        ...
        boolean continueFruit = false;
        //TODO add cone and make this true ^^
            ...
            if (Main.generateRandomInt(0, 35) == 0) {
                ...
                if (!(Main.isTheSameBlockNear(worldIn, pos, false))) {
                    ...
                    if(Main.getStateForSameNearBlock(worldIn,pos).toString().contains("variant=0")){
                        ...
                        if(gen==0){
                            worldIn.setBlockState(pos, ModBlocks.PINE_LEAVES.getDefaultState().withProperty(VARIANT, 1));
                        }else{
                            worldIn.setBlockState(pos, ModBlocks.PINE_LEAVES.getDefaultState().withProperty(VARIANT, 3));
                        }
                    }else{
                        worldIn.setBlockState(pos, ModBlocks.PINE_LEAVES.getDefaultState().withProperty(VARIANT, 2));
                    }
                }
            }
        }
    ...
    public void normalFruitGen(World worldIn,BlockPos pos){
        ...
        if (gen == 0) {
            ...
            if (worldIn.getBlockState(pos).toString().contains("variant=0")) {
                ...
                updateVariant(worldIn, pos);
                if (BlockVariant != 0) {
                    worldIn.setBlockState(pos, ModBlocks.PINE_LEAVES.getDefaultState().withProperty(VARIANT, 0));
