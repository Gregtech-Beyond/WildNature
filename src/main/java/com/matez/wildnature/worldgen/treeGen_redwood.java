...
public class treeGen_redwood extends WorldGenAbstractTree
{
    private Random rand;
    private World world;
    ...
    int leafDistanceLimit = 7;
    List<treeGen_redwood.FoliageCoordinates> foliageCoords;

    public treeGen_redwood(boolean notify)
    {
        super(notify);
    }
    ...
    void generateLeafNodeList()
    {
        ...
        this.foliageCoords = Lists.<treeGen_redwood.FoliageCoordinates>newArrayList();
        this.foliageCoords.add(new treeGen_redwood.FoliageCoordinates(this.basePos.up(k), j));
            ...
            if (f >= 0.0F)
            {
                for (int l = 0; l < i; ++l)
                {
                    ...
                    if (this.checkBlockLine(blockpos, blockpos1) == -1)
                    {
                        ...
                        {
                            this.foliageCoords.add(new treeGen_redwood.FoliageCoordinates(blockpos, blockpos2.getY()));
                        }
                    }
                }
            }
    ...
    void generateLeafNode(BlockPos pos)
    {
        ...
        {
            this.crosSection(pos.up(i), this.leafSize(i), ModBlocks.REDWOOD_LEAVES.getDefaultState().withProperty(BlockLeaves.CHECK_DECAY, Boolean.valueOf(false)));
        }
    }
    ...
    void generateLeaves()
    {
        for (treeGen_redwood.FoliageCoordinates worldgenbigtree$foliagecoordinates : this.foliageCoords)
        {
    ...
    void generateTrunk()
    {
        ...
        Block block = ModBlocks.REDWOOD_LOG;
        ...
        if(!(chance==100)){
            ...
            if((gen<=chance)){
                ...
                if (state.getBlock().isAir(state, world, blockPos) || state.getBlock().isLeaves(state, world, blockPos))
                {
                    ...
                    setBlockAndNotifyAdequately(world, blockPos, ModBlocks.REDWOOD_LOG.getDefaultState());
                }
            }
        }else {
    ...
    void generateLeafNodeBases()
    {
        for (treeGen_redwood.FoliageCoordinates worldgenbigtree$foliagecoordinates : this.foliageCoords)
        {
            ...
            {
                this.limb(blockpos, worldgenbigtree$foliagecoordinates, ModBlocks.REDWOOD_LOG);
            }
        }
    }
    ...
    private boolean validTreeLocation()
    {
        ...
        boolean isSoil = state.getBlock().canSustainPlant(state, this.world, down, net.minecraft.util.EnumFacing.UP, ((net.minecraft.block.BlockBush) ModBlocks.REDWOOD_SAPLING));
