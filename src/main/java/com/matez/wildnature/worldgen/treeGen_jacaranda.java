...
public class treeGen_jacaranda extends WorldGenAbstractTree
{

    Block LOG = ModBlocks.JACARANDA_LOG;
    Block LEAVES = ModBlocks.JACARANDA_LEAVES;
    private Random rand;
    private World world;
    private BlockPos basePos = BlockPos.ORIGIN;
    int heightLimit = Main.generateRandomInt(9,10);
    int height = Main.generateRandomInt(9,10);
    ...
    int leafDistanceLimit = Main.generateRandomInt(6,9);
    List<treeGen_jacaranda.FoliageCoordinates> foliageCoords;

    public treeGen_jacaranda(boolean notify)
    {
        super(notify);
    }
    ...
    void generateLeafNodeList()
    {
        ...
        this.foliageCoords = Lists.<treeGen_jacaranda.FoliageCoordinates>newArrayList();
        this.foliageCoords.add(new treeGen_jacaranda.FoliageCoordinates(this.basePos.up(k), j));
            ...
            if (f >= 0.0F)
            {
                for (int l = 0; l < i; ++l)
                {
                    ...
                    if (this.checkBlockLine(blockpos, blockpos1) == -1)
                    {
                        ...
                        {
                            this.foliageCoords.add(new treeGen_jacaranda.FoliageCoordinates(blockpos, blockpos2.getY()));
                        }
                    }
                }
            }
    ...
    void generateLeafNode(BlockPos pos)
    {
        ...
        {
            this.crosSection(pos.up(i), this.leafSize(i), LEAVES.getDefaultState().withProperty(BlockLeaves.CHECK_DECAY, Boolean.valueOf(false)));
        }
    }
    ...
    void generateLeaves()
    {
        for (treeGen_jacaranda.FoliageCoordinates treeGen_hazel$foliagecoordinates : this.foliageCoords)
        {
            this.generateLeafNode(treeGen_hazel$foliagecoordinates);
        }
    }
    ...
    void generateLeafNodeBases()
    {
        for (treeGen_jacaranda.FoliageCoordinates treeGen_hazel$foliagecoordinates : this.foliageCoords)
        {
            int i = treeGen_hazel$foliagecoordinates.getBranchBase();
            BlockPos blockpos = new BlockPos(this.basePos.getX(), i, this.basePos.getZ());

            if (!blockpos.equals(treeGen_hazel$foliagecoordinates) && this.leafNodeNeedsBase(i - this.basePos.getY()))
            {
                this.limb(blockpos, treeGen_hazel$foliagecoordinates, LOG);
            }
        }
    }
