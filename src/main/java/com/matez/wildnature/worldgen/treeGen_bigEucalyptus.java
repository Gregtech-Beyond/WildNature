...
public class treeGen_bigEucalyptus extends WorldGenAbstractTree
{
    private Random rand;
    private World world;
    ...
    double leafDensity = 0.6D;
    int trunkSize = 1;
    int heightLimitLimit = 25;
    /** Sets the distance limit for how far away the generator will populate leaves from the base leaf node. */
    int leafDistanceLimit = 6;
    List<treeGen_bigEucalyptus.FoliageCoordinates> foliageCoords;

    private static final IBlockState LOG = ModBlocks.EUCALYPTUS_LOG.getDefaultState();
    private static final IBlockState LEAVES = ModBlocks.EUCALYPTUS_LEAVES.getDefaultState().withProperty(LeavesBase.CHECK_DECAY, Boolean.valueOf(false)).withProperty(LeavesBase.DECAYABLE, Boolean.valueOf(false));

    public treeGen_bigEucalyptus(boolean notify)
    {
        super(notify);
    }
    ...
    void generateLeafNodeList()
    {
        ...
        this.foliageCoords = Lists.<treeGen_bigEucalyptus.FoliageCoordinates>newArrayList();
        this.foliageCoords.add(new treeGen_bigEucalyptus.FoliageCoordinates(this.basePos.up(k), j));
            ...
            if (f >= 0.0F)
            {
                for (int l = 0; l < i; ++l)
                {
                    ...
                    if (this.checkBlockLine(blockpos, blockpos1) == -1)
                    {
                        ...
                        {
                            this.foliageCoords.add(new treeGen_bigEucalyptus.FoliageCoordinates(blockpos, blockpos2.getY()));
                        }
                    }
                }
            }
    ...
    void generateLeaves()
    {
        for (treeGen_bigEucalyptus.FoliageCoordinates worldgenbigtree$foliagecoordinates : this.foliageCoords)
        {
        ...
        if(!(chance==100)){
            ...
            if((gen<=chance)){
                ...
                if (state.getBlock().isAir(state, world, blockPos) || state.getBlock().isLeaves(state, world, blockPos))
                {
                    ...
                    setBlockAndNotifyAdequately(world, blockPos, LOG);
                    return true;
                }
            }
        }else {
    ...
    void generateTrunk()
    {
        ...
        int cd = 70;
        while(true){
            generateBlock(blockpos.south().up(ab),world,cd);;
            generateBlock(blockpos.west().up(ab),world,cd);
            generateBlock(blockpos.north().up(ab),world,cd);
            generateBlock(blockpos.east().up(ab),world,cd);
            cd = cd-30;
            ab++;
            if(cd==10){
                break;
    ...
    private boolean validTreeLocation()
    {
        ...
        boolean isSoil = state.getBlock().canSustainPlant(state, this.world, down, net.minecraft.util.EnumFacing.UP, ((BlockBush) ModBlocks.EUCALYPTUS_SAPLING));
