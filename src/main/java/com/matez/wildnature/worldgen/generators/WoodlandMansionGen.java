...
import com.matez.wildnature.worldgen.generators.StructureStartGen;
...
public class WoodlandMansionGen extends MapGenStructure
{
    ...
    public WoodlandMansionGen(IChunkGenerator providerIn)
    {
    ...
    {
        return new WoodlandMansionGen.Start(this.world, (ChunkGeneratorOverworld)this.provider, this.rand, chunkX, chunkZ);
    }
            ...
            for (int j = structurebb.minX; j <= structurebb.maxX; ++j)
            {
                for (int k = structurebb.minZ; k <= structurebb.maxZ; ++k)
                {
                    ...
                    if (!worldIn.isAirBlock(blockpos) && this.boundingBox.isVecInside(blockpos))
                    {
                        ...
                        {
                            if (((StructureComponentGen)structurecomponent).boundingBox.isVecInside(blockpos))
                            {
                                flag = true;
                                break;
                            }
