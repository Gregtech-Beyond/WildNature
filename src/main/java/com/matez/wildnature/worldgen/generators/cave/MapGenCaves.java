package com.matez.wildnature.worldgen.generators.cave;
...
public class MapGenCaves extends MapGenBase
{
            ...
            if (flag2 || random.nextInt(4) != 0)
            {
                ...
                if (p_180702_6_ >= d0 - 16.0D - d2 * 2.0D && p_180702_10_ >= d1 - 16.0D - d2 * 2.0D && p_180702_6_ <= d0 + 16.0D + d2 * 2.0D && p_180702_10_ <= d1 + 16.0D + d2 * 2.0D)
                {
                    ...
                    if (!flag3)
                    {
                        ...
                        {
                            double d10 = ((double)(j3 + p_180702_3_ * 16) + 1D - p_180702_6_) / d2;

                            for (int i2 = i3; i2 < i1; ++i2)
                            {
                                double d8 = ((double)(i2 + p_180702_4_ * 16) + 1D - p_180702_10_) / d2;
                                boolean flag1 = false;
                                ...
                                if (d10 * d10 + d8 * d8 < 1.0D)
                                {
                                    ...
                                    {
                                        double d9 = ((double)(j2 - 1) + 1D - p_180702_8_) / d3;
    ...
    protected void recursiveGenerate(World worldIn, int chunkX, int chunkZ, int originalX, int originalZ, ChunkPrimer chunkPrimerIn)
    {
        ...
        for (int j = 0; j < i; ++j)
        {
            double d0 = (double)(chunkX * 16 + this.rand.nextInt(256));
            double d1 = (double)this.rand.nextInt(this.rand.nextInt(120) + 8);
            double d2 = (double)(chunkZ * 16 + this.rand.nextInt(256));
            int k = 1;
