...
public class BigTree extends WorldGenAbstractTree
{
    private Random rand;
    private World world;
    private BlockPos basePos = BlockPos.ORIGIN;
    IBlockState LEAVES, LOG, SAPLING;
    int heightLimit;
    int height;
    ...
    public BigTree(boolean notify, int trunkHeight, int sizeTrunk, double densityLeaf, IBlockState log, IBlockState leaves, IBlockState sapling)
    {
        super(notify);
        heightLimit = trunkHeight+2;
        heightLimitLimit = trunkHeight+1;
        height = trunkHeight;
        trunkSize = sizeTrunk;
        leafDensity = densityLeaf;
        LOG = log;
        LEAVES = leaves;
        SAPLING = sapling;
    }
    ...
    void generateLeafNodeList()
    {
        ...
        this.foliageCoords.add(new BigTree.FoliageCoordinates(this.basePos.up(k), j));
            ...
            if (f >= 0.0F)
            {
                for (int l = 0; l < i; ++l)
                {
                    ...
                    if (this.checkBlockLine(blockpos, blockpos1) == -1)
                    {
                        ...
                        {
                            this.foliageCoords.add(new BigTree.FoliageCoordinates(blockpos, blockpos2.getY()));
                        }
                    }
                }
            }
    ...
    void limb(BlockPos p_175937_1_, BlockPos p_175937_2_, Block p_175937_3_)
    {
        ...
        for (int j = 0; j <= i; ++j)
        {
            ...
            this.setBlockAndNotifyAdequately(this.world, blockpos1, LOG);
        }
    }
    ...
    void generateLeaves()
    {
        for (BigTree.FoliageCoordinates BigTree$foliagecoordinates : this.foliageCoords)
        {
            this.generateLeafNode(BigTree$foliagecoordinates);
        }
    }
    ...
    void generateLeafNodeBases()
    {
        for (BigTree.FoliageCoordinates BigTree$foliagecoordinates : this.foliageCoords)
        {
            ...
            if (!blockpos.equals(BigTree$foliagecoordinates) && this.leafNodeNeedsBase(i - this.basePos.getY()))
            {
                this.limb(blockpos, BigTree$foliagecoordinates, LOG.getBlock());
            }
        }
    }
    ...
    private boolean validTreeLocation()
    {
        ...
        boolean isSoil = state.getBlock().canSustainPlant(state, this.world, down, net.minecraft.util.EnumFacing.UP, ((IPlantable)SAPLING.getBlock()));
