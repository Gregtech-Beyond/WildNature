...
public class treeGen_ebony extends WorldGenAbstractTree
{
    private Random rand;
    private World world;
    ...
    double leafDensity = 0.5D;
    int trunkSize = 2;
    int heightLimitLimit = 16;
    /** Sets the distance limit for how far away the generator will populate leaves from the base leaf node. */
    int leafDistanceLimit = 8;
    List<treeGen_ebony.FoliageCoordinates> foliageCoords;
    private static final IBlockState LOG = ModBlocks.EBONY_LOG.getDefaultState();
    private static final IBlockState LEAVES = ModBlocks.EBONY_LEAVES.getDefaultState().withProperty(LeavesBase.CHECK_DECAY, Boolean.valueOf(false)).withProperty(LeavesBase.DECAYABLE, Boolean.valueOf(false));
    public treeGen_ebony(boolean notify)
    {
        super(notify);
    }
    ...
    void generateLeafNodeList()
    {
        ...
        this.foliageCoords = Lists.<treeGen_ebony.FoliageCoordinates>newArrayList();
        this.foliageCoords.add(new treeGen_ebony.FoliageCoordinates(this.basePos.up(k), j));
            ...
            if (f >= 0.0F)
            {
                for (int l = 0; l < i; ++l)
                {
                    ...
                    if (this.checkBlockLine(blockpos, blockpos1) == -1)
                    {
                        ...
                        {
                            this.foliageCoords.add(new treeGen_ebony.FoliageCoordinates(blockpos, blockpos2.getY()));
                        }
                    }
                }
            }
    ...
    void generateLeaves()
    {
        for (treeGen_ebony.FoliageCoordinates worldgenbigtree$foliagecoordinates : this.foliageCoords)
        {
    ...
    void generateTrunk()
    {
        ...
        int cd = 90;
        while(true){
            generateBlock(blockpos.south(2).up(ab),world,cd);;
            generateBlock(blockpos.west().up(ab),world,cd);
            generateBlock(blockpos.north().up(ab),world,cd);
            generateBlock(blockpos.east(2).up(ab),world,cd);
            cd = cd-30;
            ab++;
            if(cd==30){
                break;
    ...
    private boolean validTreeLocation()
    {
        ...
        boolean isSoil = state.getBlock().canSustainPlant(state, this.world, down, net.minecraft.util.EnumFacing.UP, ((net.minecraft.block.BlockBush) ModBlocks.EBONY_SAPLING));
