...
public class BiomeDecoratorFlowerField extends BiomeDecorator {
        ...
        public ArrayList<IBlockState> grass = new ArrayList<>();
        ...
        protected void genDecorations(Biome biomeIn, World worldIn, Random random)
        {
            ...
            if(net.minecraftforge.event.terraingen.TerrainGen.decorate(worldIn, random, forgeChunkPos, net.minecraftforge.event.terraingen.DecorateBiomeEvent.Decorate.EventType.GRASS)) {
                for (int i3 = 0; i3 < this.grassPerChunk; ++i3) {
                    ...
                    IBlockState grassState = null;
                    if(abcd>=grass.size()){
                        abcd=0;
                    }
                    grassState = grass.get(abcd);
                    //System.out.println("GRASSSTATE " +grassState.getValue(Blocks.RED_FLOWER.getTypeProperty()));

                    if (k14 > 0) {
                        int l17 = random.nextInt(k14);
                        new WorldGenGrass(grassState).generate(worldIn, random, this.chunkPos.add(j7, l17, i11));
                    }
                    abcd++;
                }
            }
